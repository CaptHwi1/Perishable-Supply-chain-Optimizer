    ...
    def run_simulation(self):
        ...
        for prod_name, prod_data in self.products.items():
            ...
            for current_day in range(1, total_days + 1):
                ...
                # Process distributors in priority order (by policy_days) - PRECEDENCE RELATIONSHIP
...
class SupplyChainApp:
    def __init__(self, root):
        ...
        # Theme variables - Redwood design system inspired
        self.is_dark_mode = True
        self.colors = self._get_redwood_colors()
    ...
    def _get_redwood_colors(self):
        """Return Redwood design system inspired color scheme based on theme"""
        if self.is_dark_mode:
            return {
                'bg': '#1a1c1e',  # Deep charcoal
                'fg': '#e0e0e0',  # Light gray text
                'header_bg': '#d32f2f',  # Redwood red
                'frame_bg': '#262a2e',  # Dark slate
                'button_bg': '#d32f2f',  # Redwood red
                'button_fg': '#ffffff',  # White text
                'entry_bg': '#33373b',  # Darker slate
                'entry_fg': '#e0e0e0',  # Light gray text
                'tree_bg': '#262a2e',  # Dark slate
                'tree_field': '#33373b',  # Darker slate
                'tree_text': '#e0e0e0',  # Light gray text
                'accent': '#ff6b6b',  # Bright accent red
                'highlight': '#ff9a9a'  # Highlight color
            }
        else:
            return {
                'bg': '#f8f9fa',  # Light gray background
                'fg': '#212529',  # Dark gray text
                'header_bg': '#c62828',  # Darker redwood red
                'frame_bg': '#ffffff',  # White
                'button_bg': '#d32f2f',  # Redwood red
                'button_fg': '#ffffff',  # White text
                'entry_bg': '#ffffff',  # White
                'entry_fg': '#212529',  # Dark gray text
                'tree_bg': '#ffffff',  # White
                'tree_field': '#f8f9fa',  # Light gray
                'tree_text': '#212529',  # Dark gray text
                'accent': '#ef5350',  # Accent red
                'highlight': '#ffcdd2'  # Highlight color
            }
    ...
    def create_widgets(self):
        ...
        # Custom styles for Redwood theme
        ...
        style.map("TButton",
                  ...
                  background=[('pressed', '#9a0007'), ('active', '#b71c1c')])
        ...
        # Control panel using grid to avoid pack/grid conflict
        ...
        ctrl_frame.columnconfigure(5, weight=1)
        
        ttk.Label(ctrl_frame, text="Simulation Days:").grid(row=0, column=0, sticky="e", padx=(0, 5))
        sim_days_entry = ttk.Entry(ctrl_frame, textvariable=self.sim_days, width=8)
        sim_days_entry.grid(row=0, column=1, sticky="w", padx=(0, 10))
        
        ttk.Label(ctrl_frame, text="Transport Rate ($/km/unit):").grid(row=0, column=2, sticky="e", padx=(0, 5))
        transport_entry = ttk.Entry(ctrl_frame, textvariable=self.transport_rate, width=8)
        transport_entry.grid(row=0, column=3, sticky="w", padx=(0, 10))
        
        apply_btn = ttk.Button(ctrl_frame, text="Apply Settings", command=self.apply_settings)
        apply_btn.grid(row=0, column=4, padx=(0, 10))

        # Products Section
        ttk.Label(products_frame, text="Products", font=("", 11, "bold")).grid(row=1, column=0, columnspan=2, sticky="w", padx=10, pady=(20, 5))
        
        prod_frame = ttk.Frame(products_frame)
        prod_frame.grid(row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=5)
        prod_frame.columnconfigure(1, weight=1)
        prod_frame.columnconfigure(3, weight=1)
        prod_frame.columnconfigure(5, weight=1)
        
        ttk.Label(prod_frame, text="Name:").grid(row=0, column=0, sticky="e", padx=(0, 5))
        self.prod_name_entry = ttk.Entry(prod_frame, textvariable=self.prod_name_var, width=10)
        self.prod_name_entry.grid(row=0, column=1, sticky="w", padx=(0, 10))
        
        ttk.Label(prod_frame, text="Shelf Life:").grid(row=0, column=2, sticky="e", padx=(0, 5))
        self.prod_shelf_life_entry = ttk.Entry(prod_frame, textvariable=self.prod_shelf_life_var, width=6)
        self.prod_shelf_life_entry.grid(row=0, column=3, sticky="w", padx=(0, 10))
        
        ttk.Button(prod_frame, text="Add Product", command=self.add_product).grid(row=0, column=4, padx=(0, 10))
        
        # Product listbox
        self.prod_listbox = tk.Listbox(products_frame, height=6, bg=self.colors['entry_bg'], fg=self.colors['entry_fg'], selectbackground=self.colors['accent'])
        self.prod_listbox.grid(row=3, column=0, columnspan=2, sticky="ew", padx=10, pady=2)
        
        # Fixed button frame to ensure proper layout
        btn_prod_frame = ttk.Frame(products_frame)
        btn_prod_frame.grid(row=4, column=0, columnspan=2, padx=10, pady=2)
        ttk.Button(btn_prod_frame, text="Remove Selected Product", 
                  command=self.remove_product).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(btn_prod_frame, text="Clear All Products", 
                  command=self.clear_all_products).pack(side=tk.LEFT)
        ...
        ttk.Label(dist_frame, text="Distributors", font=("", 11, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", padx=10, pady=(20, 5))
        
        dist_setup_frame = ttk.Frame(dist_frame)
        dist_setup_frame.grid(row=1, column=0, columnspan=2, sticky="ew", padx=10, pady=5)
        dist_setup_frame.columnconfigure(1, weight=1)
        dist_setup_frame.columnconfigure(3, weight=1)
        dist_setup_frame.columnconfigure(5, weight=1)
        dist_setup_frame.columnconfigure(7, weight=1)
        
        ttk.Label(dist_setup_frame, text="Name:").grid(row=0, column=0, sticky="e", padx=(0, 5))
        self.dist_name_entry = ttk.Entry(dist_setup_frame, textvariable=self.dist_name_var, width=10)
        self.dist_name_entry.grid(row=0, column=1, sticky="w", padx=(0, 10))
        
        ttk.Label(dist_setup_frame, text="Policy:").grid(row=0, column=2, sticky="e", padx=(0, 5))
        self.dist_policy_spin = ttk.Spinbox(dist_setup_frame, from_=1, to=30, textvariable=self.dist_policy_var, width=5)
        self.dist_policy_spin.grid(row=0, column=3, sticky="w", padx=(0, 10))
        
        ttk.Label(dist_setup_frame, text="Prop:").grid(row=0, column=4, sticky="e", padx=(0, 5))
        ...
        self.dist_prop_spin.grid(row=0, column=5, sticky="w", padx=(0, 10))
        
        ttk.Label(dist_setup_frame, text="Dist(km):").grid(row=0, column=6, sticky="e", padx=(0, 5))
        self.dist_dist_entry = ttk.Entry(dist_setup_frame, textvariable=self.dist_dist_var, width=6)
        self.dist_dist_entry.grid(row=0, column=7, sticky="w", padx=(0, 10))
        
        ttk.Button(dist_setup_frame, text="Add Distributor", command=self.add_distributor).grid(row=0, column=8, padx=(0, 10))
        
        # Product selection for distributor - now with dropdown
        self.prod_selection_frame = ttk.Frame(dist_frame)
        self.prod_selection_frame.grid(row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=5)
        
        ttk.Label(self.prod_selection_frame, text="Select products for distributor:").pack(anchor="w")
        
        # Create a frame for the product selection grid
        self.prod_grid_frame = ttk.Frame(self.prod_selection_frame)
        self.prod_grid_frame.pack(fill="x", padx=10, pady=5)
        
        self.prod_checkboxes = []
        self.prod_vars = []
        
        # Flexible day configuration for distributor
        self.days_config_frame = ttk.Frame(dist_frame)
        self.days_config_frame.grid(row=3, column=0, columnspan=2, sticky="ew", padx=10, pady=5)
        ttk.Label(self.days_config_frame, text="Days distributor buys:").pack(anchor="w")
        
        # Day configuration frame
        self.day_config_frame = ttk.Frame(self.days_config_frame)
        self.day_config_frame.pack(fill="x", padx=10)
        ...
        for i, day_name in enumerate(self.day_names):
            ...
            cb = ttk.Checkbutton(self.day_config_frame, text=day_name, variable=var)
            cb.pack(side=tk.LEFT, padx=5)

        # Distributor list
        self.dist_listbox = tk.Listbox(dist_frame, height=6, bg=self.colors['entry_bg'], fg=self.colors['entry_fg'], selectbackground=self.colors['accent'])
        self.dist_listbox.grid(row=4, column=0, columnspan=2, sticky="ew", padx=10, pady=2)
        
        # Fixed button frame for distributors
        btn_dist_frame = ttk.Frame(dist_frame)
        btn_dist_frame.grid(row=5, column=0, columnspan=2, padx=10, pady=2)
        ttk.Button(btn_dist_frame, text="Remove Selected Distributor", 
                  command=self.remove_distributor).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(btn_dist_frame, text="Clear All Distributors", 
                  command=self.clear_all_distributors).pack(side=tk.LEFT)
        ...
        ttk.Label(prod_plan_frame, text="Production Plan", font=("", 11, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", padx=10, pady=(20, 5))
        ...
        ttk.Label(input_frame, text="Product:").grid(row=0, column=0, sticky="e", padx=(0, 5))
        self.prod_combo = ttk.Combobox(input_frame, textvariable=self.prod_var, state="readonly", width=12)
        self.prod_combo.grid(row=0, column=1, sticky="w", padx=(0, 10))
        
        ttk.Label(input_frame, text="Day:").grid(row=0, column=2, sticky="e", padx=(0, 5))
        self.prod_day_entry = ttk.Entry(input_frame, textvariable=self.prod_day_var, width=6)
        self.prod_day_entry.grid(row=0, column=3, sticky="w", padx=(0, 10))
        
        ttk.Label(input_frame, text="Qty:").grid(row=0, column=4, sticky="e", padx=(0, 5))
        self.prod_qty_entry = ttk.Entry(input_frame, textvariable=self.prod_qty_var, width=8)
        self.prod_qty_entry.grid(row=0, column=5, sticky="w", padx=(0, 10))
        
        ttk.Button(input_frame, text="Add", command=self.add_production).grid(row=0, column=6, padx=(0, 5))
        ttk.Button(input_frame, text="Remove", command=self.remove_production).grid(row=0, column=7, padx=(0, 10))
        ...
        self.prod_tree.grid(row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=5)

        # Buttons frame
        btn_frame = ttk.Frame(setup_notebook)
        setup_notebook.add(btn_frame, text="Actions")
        
        ttk.Button(btn_frame, text="Run Simulation", command=self.run_simulation).pack(pady=20, ipadx=20, ipady=10)
        ttk.Button(btn_frame, text="Optimize Production", command=self.optimize_production).pack(pady=20, ipadx=20, ipady=10)
        ttk.Button(btn_frame, text="Export Original Results", command=self.export_original_results).pack(pady=20, ipadx=20, ipady=10)
        ttk.Button(btn_frame, text="Export Optimized Plan", command=self.export_optimized_plan).pack(pady=20, ipadx=20, ipady=10)
        ttk.Button(btn_frame, text="Export All", command=self.export_all).pack(pady=20, ipadx=20, ipady=10)
    ...
    def apply_settings(self):
        """Apply settings without recreating the whole UI"""
        self.update_ui()
        messagebox.showinfo("Settings Applied", f"Simulation days set to {self.sim_days.get()}")
    ...
    def update_ui(self):
        ...
        for widget in self.prod_grid_frame.winfo_children():
            widget.destroy()
            
        # Create product checkboxes in a grid
        for i, prod_name in enumerate(self.products.keys()):
            var = tk.BooleanVar()
            self.prod_vars.append(var)
            cb = ttk.Checkbutton(self.prod_grid_frame, text=prod_name, variable=var)
    ...
    def add_product(self):
        ...
        if name in self.products:
            return messagebox.showwarning("Duplicate", f"Product '{name}' already exists.")
    ...
    def remove_product(self):
        """Fixed function to properly remove selected product"""
        sel = self.prod_listbox.curselection()
        if not sel:
            return messagebox.showwarning("Selection", "Please select a product to remove.")
        ...
        # Confirm deletion
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete product '{prod_name}'?"):
            self.prod_listbox.delete(idx)
            if prod_name in self.products:
                del self.products[prod_name]
            self.update_ui()
            messagebox.showinfo("Success", f"Product '{prod_name}' has been removed.")

    def clear_all_products(self):
        """Clear all products with confirmation"""
        if not self.products:
            return messagebox.showinfo("No Products", "No products to clear.")
            
        if messagebox.askyesno("Clear All", "Are you sure you want to remove ALL products? This cannot be undone."):
            self.products.clear()
            self.prod_listbox.delete(0, tk.END)
            self.update_ui()
            messagebox.showinfo("Success", "All products have been cleared.")
    ...
    def add_distributor(self):
        ...
        purchase_pattern = [var.get() for var in self.day_vars]  # Weekly pattern (Mon-Sun)
        full_purchase_days = []
        for day in range(1, self.sim_days.get() + 1):
            weekday_index = (day - 1) % 7  # 0=Mon, 1=Tue, ..., 6=Sun
            full_purchase_days.append(purchase_pattern[weekday_index])
    ...
    def remove_distributor(self):
        ...
        if not sel:
            return messagebox.showwarning("Selection", "Please select a distributor to remove.")
            
        idx = sel[0]
        item_text = self.dist_listbox.get(idx)
        dist_name = item_text.split()[0]
        
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete distributor '{dist_name}'?"):
            self.dist_listbox.delete(idx)
            if idx < len(self.distributors):
                del self.distributors[idx]
            self.update_ui()
            messagebox.showinfo("Success", f"Distributor '{dist_name}' has been removed.")

    def clear_all_distributors(self):
        """Clear all distributors with confirmation"""
        if not self.distributors:
            return messagebox.showinfo("No Distributors", "No distributors to clear.")
            
        if messagebox.askyesno("Clear All", "Are you sure you want to remove ALL distributors? This cannot be undone."):
            self.distributors.clear()
            self.dist_listbox.delete(0, tk.END)
            self.update_ui()
            messagebox.showinfo("Success", "All distributors have been cleared.")
    ...
    def remove_production(self):
        ...
        if not selected:
            return messagebox.showwarning("Selection", "Please select a production entry to remove.")
