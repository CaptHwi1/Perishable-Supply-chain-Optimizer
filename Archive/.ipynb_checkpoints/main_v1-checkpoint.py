...
class SupplyChainApp:
    ...
    def create_widgets(self):
        ...
        # Theme toggle
        theme_btn = tk.Button(header_frame, text="☀️ Light Mode" if self.is_dark_mode else "🌙 Dark Mode",
                             command=self.toggle_theme, bg=self.colors['header_bg'], 
        ...
        notebook.add(setup_frame, text="Configuration")
        ...
        ttk.Button(ctrl_frame, text="Apply Settings", command=self.update_ui).pack(side=tk.LEFT, padx=10)

        # Products Section
        ttk.Label(setup_frame, text="Products", font=("", 11, "bold")).pack(anchor="w", padx=10, pady=(20, 5))
        
        prod_frame = ttk.Frame(setup_frame)
        prod_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(prod_frame, text="Name:").grid(row=0, column=0, sticky="w", padx=2)
        self.prod_name_entry = ttk.Entry(prod_frame, textvariable=self.prod_name_var, width=10)
        self.prod_name_entry.grid(row=0, column=1, padx=2)
        
        ttk.Label(prod_frame, text="Shelf Life:").grid(row=0, column=2, sticky="w", padx=2)
        self.prod_shelf_life_entry = ttk.Entry(prod_frame, textvariable=self.prod_shelf_life_var, width=6)
        self.prod_shelf_life_entry.grid(row=0, column=3, padx=2)
        
        ttk.Button(prod_frame, text="Add Product", command=self.add_product).grid(row=0, column=4, padx=5)
        
        self.prod_listbox = tk.Listbox(setup_frame, height=3)
        self.prod_listbox.pack(fill="x", padx=10, pady=2)
        ttk.Button(setup_frame, text="Remove Selected Product", 
                  command=self.remove_product).pack(padx=10, pady=2)

        # Distributors Section
        ttk.Label(setup_frame, text="Distributors", font=("", 11, "bold")).pack(anchor="w", padx=10, pady=(20, 5))
        
        dist_setup_frame = ttk.Frame(setup_frame)
        dist_setup_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(dist_setup_frame, text="Name:").grid(row=0, column=0, sticky="w", padx=2)
        self.dist_name_entry = ttk.Entry(dist_setup_frame, textvariable=self.dist_name_var, width=10)
        self.dist_name_entry.grid(row=0, column=1, padx=2)
        
        ttk.Label(dist_setup_frame, text="Policy:").grid(row=0, column=2, sticky="w", padx=2)
        self.dist_policy_spin = ttk.Spinbox(dist_setup_frame, from_=1, to=30, textvariable=self.dist_policy_var, width=5)
        self.dist_policy_spin.grid(row=0, column=3, padx=2)
        
        ttk.Label(dist_setup_frame, text="Prop:").grid(row=0, column=4, sticky="w", padx=2)
        ...
        self.dist_prop_spin.grid(row=0, column=5, padx=2)
        
        ttk.Label(dist_setup_frame, text="Dist(km):").grid(row=0, column=6, sticky="w", padx=2)
        self.dist_dist_entry = ttk.Entry(dist_setup_frame, textvariable=self.dist_dist_var, width=6)
        self.dist_dist_entry.grid(row=0, column=7, padx=2)
        
        ttk.Button(dist_setup_frame, text="Add Distributor", command=self.add_distributor).grid(row=0, column=8, padx=5)
        
        # Product selection for distributor
        self.prod_selection_frame = ttk.Frame(setup_frame)
        ...
        # Distributor list
        self.dist_listbox = tk.Listbox(setup_frame, height=4)
        self.dist_listbox.pack(fill="x", padx=10, pady=2)
        ttk.Button(setup_frame, text="Remove Selected Distributor", 
                  command=self.remove_distributor).pack(padx=10, pady=2)

        # Production Plan Section
        ttk.Label(setup_frame, text="Production Plan", font=("", 11, "bold")).pack(anchor="w", padx=10, pady=(20, 5))
        
        prod_plan_frame = ttk.Frame(setup_frame)
        prod_plan_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(prod_plan_frame, text="Product:").grid(row=0, column=0, sticky="w", padx=2)
        self.prod_combo = ttk.Combobox(prod_plan_frame, textvariable=self.prod_var, state="readonly", width=12)
        self.prod_combo.grid(row=0, column=1, padx=2)
        
        ttk.Label(prod_plan_frame, text="Day:").grid(row=0, column=2, sticky="w", padx=2)
        self.prod_day_entry = ttk.Entry(prod_plan_frame, textvariable=self.prod_day_var, width=6)
        self.prod_day_entry.grid(row=0, column=3, padx=2)
        
        ttk.Label(prod_plan_frame, text="Qty:").grid(row=0, column=4, sticky="w", padx=2)
        self.prod_qty_entry = ttk.Entry(prod_plan_frame, textvariable=self.prod_qty_var, width=8)
        self.prod_qty_entry.grid(row=0, column=5, padx=2)
        
        ttk.Button(prod_plan_frame, text="Add", command=self.add_production).grid(row=0, column=6, padx=2)
        ttk.Button(prod_plan_frame, text="Remove", command=self.remove_production).grid(row=0, column=7, padx=2)
        
        # Production tree
        self.prod_tree = ttk.Treeview(setup_frame, columns=("product", "day", "qty"), show="headings", height=6)
        ...
        ttk.Button(btn_frame, text="Run Simulation", command=self.run_simulation).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="Optimize Production", command=self.optimize_production).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="Export Original Results", command=self.export_original_results).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="Export Optimized Plan", command=self.export_optimized_plan).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="Export All", command=self.export_all).pack(side=tk.LEFT, padx=10)
    ...
    def toggle_theme(self):
        ...
        self.colors = self._get_colors()
        self.root.destroy()  # Destroy and recreate to properly apply theme
        self.__init__(tk.Tk())
    ...
    def add_product(self):
        ...
        self.prod_listbox.insert(tk.END, f"{name} (SL={sl})")
    ...
    def add_distributor(self):
        ...
        self.distributors.append({
            ...
            'purchase_days': [True]*self.sim_days.get(),
            'preferred_products': selected_products
        })
        
        self.dist_listbox.insert(tk.END, f"{name} (Policy={policy}, Products={len(selected_products)})")
